...
export function MbzUpload({ onUpload, onScreenshot, onHtmlUpload, mbzInfo, pendingScreenshot, htmlPageInfo }: MbzUploadProps) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      {/* Hidden inputs */}
      <input ref={mbzInputRef} type="file" accept=".mbz" onChange={handleMbzChange} className="hidden" />
      <input ref={screenshotInputRef} type="file" accept="image/*" onChange={handleScreenshotChange} className="hidden" />
      <input ref={htmlInputRef} type="file" accept=".html,.htm" onChange={handleHtmlChange} className="hidden" />
